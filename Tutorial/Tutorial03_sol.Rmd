...
At first glance, it seems that 807 uniquely represents the flight from __EWR___ to __ATL__ by carrier ___Delta___. To confirm this, create a new dataset `f1e` that is `f1d` sorted by the three variables previously mentioned, all in descending order. The function `desc()` is required here. Follow this by displaying the first 18 rows of the data in `f1d`.
...
May we conclude that flight numbers can be mapped to a unique carrier? __No____ (*Yes/No*)

Why or why not?

If we know the flight number, carrier, and origin, can we know for sure what the destination will be? __No___ (*Yes/No*)
