...
Recall the Wikipedia page listing the [crime rates](https://en.wikipedia.org/wiki/List_of_United_States_cities_by_crime_rate) of US cities. This information comes from an FBI report from 2017. Suppose we wanted to investigate the effect of crime in 2017 on the population data in 2019. An HTML table, provided online, reports the [2019 population estimate](https://en.wikipedia.org/wiki/List_of_United_States_cities_by_population) of 317 US cities.

- Why is this information useful when related to violent crimes?
- Which columns do you think are useful?
- What would you expect to discover?

## Chunk 1: Web Scraping and Variable Selection

We start by web scraping the population statistics table into R using functions from `rvest`. We only select the variables of interest `2019 estimate`, `2016 land area` and `2016 population density` and the keys `City` and `State` required to link this information to violent crimes. 

```{r}
URL.US_CENSUS = "https://en.wikipedia.org/wiki/List_of_United_States_cities_by_population"
...
## Chunk 2: Population, Land Area and Density Variables to Numeric

The column names are messy and the last three columns should be numeric. The following code starts by giving the columns more succinct names and then continues to provide the necessary mutations required to convert character vectors to numeric vectors. 

```{r}
US_CENSUS2 = US_CENSUS %>%
      rename(Name=City, State=names(US_CENSUS)[2],Pop_2019=names(US_CENSUS)[3],Land=names(US_CENSUS)[4],Density_2016=names(US_CENSUS)[5]) %>%
      mutate(Pop_2019=as.numeric(str_replace_all(Pop_2019,",","")))%>%
      mutate(Land=as.numeric(str_replace_all(Land,",|km2",""))) %>%
      mutate(Density_2016=as.numeric(str_replace_all(Density_2016,",|/km2","")))

US_CENSUS$`2019estimate`[1] # Example
str_replace_all(US_CENSUS$`2019estimate`[1],",","") # Test

str(US_CENSUS2)
```

## Chunk 3: Create Change and Density (2019) Variables and Clean the Name Column
Now we have population information in 2019 and 2016. We can create a variable `Change` to represent the population change from 2016 to 2019 and a variable `Density` to represent population density in 2019. Note that the `Name` column contains unwanted footnote characters. The following code creates `Change` and `Density` variables, clean the `Name` column and keep variables we are interested in.
```{r}
US_CENSUS3 = US_CENSUS2 %>%
      mutate(Pop_2016=Density_2016*Land, Change=round((Pop_2019-Pop_2016)/Pop_2016*100,2)) %>%
      mutate(Density=round(Pop_2019/Land)) %>%
...
An article at https://www.securitysales.com/fire-intrusion/2018-safest-most-dangerous-states-us/ summarizes a study performed by [WalletHub](https://wallethub.com/edu/safest-states-to-live-in/4566/) which compared all 50 states using 48 key safety indicators. The article provides a link to the source along with lists providing the 10 most safe and 10 most dangerous states. 
...
# Reading
Remember the following lists linking to additional information on this topic.
